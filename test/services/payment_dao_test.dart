...
import 'package:offline_pos/services/payment_dao.dart';
...
void main() {
  ...
  setUp(() async {
    ...
    firstInstallmentId = await database.into(database.installments).insert(
    ...
    secondInstallmentId = await database.into(database.installments).insert(
      InstallmentsCompanion.insert(
        saleId: saleId,
        monthNumber: 2,
  ...
  Future<Installment> loadInstallment(int id) {
    return (database.select(database.installments)
          ..where((installment) => installment.id.equals(id)))
        .getSingle();
  }
  ...
  test('exact payment marks the installment paid with zero remaining', () async {
    await paymentDao.recordPayment(
      installmentId: firstInstallmentId,
      amount: 100,
      paymentDate: DateTime(2026, 1, 10),
  ...
  test('overpayment stays on its installment and displays zero remaining', () async {
    await paymentDao.recordPayment(
      installmentId: firstInstallmentId,
      amount: 120,
      paymentDate: DateTime(2026, 1, 10),
  ...
  test('multiple payments accumulate on the same installment', () async {
    ...
    expect(
      payments.every(
        (payment) => payment.installmentId == firstInstallmentId,
      ),
      isTrue,
    );
  });
  ...
  test('already paid installment rejects payment without changes', () async {
    await (database.update(database.installments)
          ..where((installment) => installment.id.equals(firstInstallmentId)))
        .write(
